...
class RealtimeV3SessionState:
    ...
    frames, restore contexts, torch checkpoint, HuBERT) without contaminating
    each other's LSTM hidden states or template cycle position.
    """
...
class RealtimeV3Worker:
    def __init__(
        ...
        self.model = self._load_torch_model(checkpoint)
    ...
    def _load_torch_model(self, checkpoint: Path | None):
    ...
    def generate_frames_from_reps(
        ...
        with torch.inference_mode():
            for rep in reps:
                ...
                audio_t = torch.from_numpy(rep_input.astype(np.float32)).to(self.v2.device)
                face_t = torch.from_numpy(context.face_input.astype(np.float32)).to(self.v2.device)
                hn_t = torch.from_numpy(np.asarray(hn_in, dtype=np.float32)).to(self.v2.device)
                cn_t = torch.from_numpy(np.asarray(cn_in, dtype=np.float32)).to(self.v2.device)
                g_t, hn_t_out, cn_t_out = self.model(audio_t, face_t, hn_t, cn_t)
                g = g_t.detach().cpu().numpy()
                hn_out = hn_t_out.detach().cpu().numpy()
                cn_out = cn_t_out.detach().cpu().numpy()
                if state is not None:
