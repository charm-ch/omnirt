#!/usr/bin/env bash
# Start MuseTalk behind a FlashTalk-compatible WebSocket.
set -euo pipefail
...
Requires the OmniRT-managed MuseTalk source checkout and MuseTalk v1.5 weights — see:
...
Environment:
  ...
  OMNIRT_MUSETALK_DEVICE_PROFILE Runtime device profile for state lookup (cuda|npu|cpu; default cuda)
  OMNIRT_MUSETALK_REPO           MuseTalk source checkout (default from OmniRT runtime home)
...
export OMNIRT_HOME="${OMNIRT_HOME:-$ROOT/.omnirt}"
RUNTIME_DEVICE="${OMNIRT_MUSETALK_DEVICE_PROFILE:-cuda}"
OMNIRT_CLI_PYTHON="${OMNIRT_CLI_PYTHON:-$ROOT/.venv/bin/python}"
if [[ ! -x "$OMNIRT_CLI_PYTHON" ]]; then
  OMNIRT_CLI_PYTHON="python3"
fi
if env_output="$("$OMNIRT_CLI_PYTHON" -m omnirt.cli.main runtime env musetalk --device "$RUNTIME_DEVICE" --shell 2>/dev/null)"; then
  set +u
  eval "$env_output"
  set -u
fi
...
# If `omnirt runtime install musetalk --device ...` has run, runtime env
# provides OMNIRT_MUSETALK_PYTHON. Keep the local backend venv as a fallback.
